...
namespace osapi
{
  ...
  void Thread::setPriority(Thread::ThreadPriority priority)
  {
    ...
    {
        priority_ = priority;   // Update attribute
        sched_param sched_p;
        sched_p.sched_priority = static_cast<int>(priority_);   // Prepare parameters

        if(pthread_setschedparam(threadId_, SCHED_RR, &sched_p) != 0) throw ThreadError();    // Use the priority
    }
  }
